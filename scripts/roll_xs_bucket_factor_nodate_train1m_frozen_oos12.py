...
# coding: utf-8
"""
**V1 no-date prompt** (`roll_xs_bucket_factor_pipeline_nodate`): train **one** calendar month only, then
**freeze** and run **N** OOS test months (test = start_train_month + k + 2).

- Factor = same as v1 pipeline: softmax expected bucket × 100/4 (see `logits_to_factor` in nodate module).
- Default OOS: **combo_cache** — one LM forward per unique `combo_key` across all OOS months, map to rows.

Does **not** modify `roll_xs_bucket_factor_pipeline_nodate.py`.

Example (Qwen2.5-32B):
  python3 -u roll_xs_bucket_factor_nodate_train1m_frozen_oos12.py --backend qwen32 --multi_gpu_auto \\
    --start_train_month 2023-01 --n_rolls 12
"""
...
import roll_xs_bucket_factor_pipeline_nodate as nd


def setup_logger(log_path: str) -> logging.Logger:
    logger = logging.getLogger("roll_xs_factor_nodate_train1m_frozen_oos12")
...
def infer_combo_to_factor(
    ...
    """One forward per unique combo_key (nodate prompt ignores calendar date in text)."""
    ...
    logger.info("Infer unique combos: %d (batch_size=%d)", len(uniq_combos), nd.BATCH_SIZE)
    out = nd.predict_factors_for_test(wrapped, synth, tokenizer, backend, pad_id, device)
...
def log_val_prediction_breakdown(
    ...
    wrapped.eval()
    ds = nd.ComboBucketTrainDataset(lab_va, tokenizer, nd.MAX_LENGTH, backend)
    loader = DataLoader(
        ds,
        batch_size=nd.BATCH_SIZE,
        shuffle=False,
        collate_fn=lambda b: nd.collate_batch(b, pad_id),
        num_workers=0,
    )
    preds_all: List[int] = []
    true_all: List[int] = []
...
def main() -> None:
    ...
    ap.add_argument("--backend", choices=list(nd.BACKENDS.keys()), default="qwen")
    ap.add_argument("--start_train_month", type=str, default=nd.START_TRAIN_MONTH)
    ap.add_argument(
        "--n_rolls",
        type=int,
        default=12,
    ...
    ap.add_argument(
        ...
        default="combo_cache",
        help="OOS: combo_cache = union unique combo_key; per_month = full forward per month.",
    )
    ap.add_argument(
        "--infer_batch_size",
        type=int,
    ...
    if args.batch_size is not None:
        nd.BATCH_SIZE = args.batch_size
    if args.grad_accum is not None:
        nd.GRAD_ACCUM = args.grad_accum
    ...
    bc = nd.BACKENDS[args.backend]
    run_id = (
        args.backend
        + "_nodate_train1m_frozen_oos"
        + str(args.n_rolls)
        + "_"
        + datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    out_dir = os.path.join(nd.BASE_DIR, "results", run_id)
    log_dir = os.path.join(nd.BASE_DIR, "logs", run_id)
    final_ckpt_dir = os.path.join(nd.BASE_DIR, "mappings", run_id, "final")
    os.makedirs(out_dir, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(final_ckpt_dir, exist_ok=True)

    logger = setup_logger(os.path.join(log_dir, "run.log"))
    rng = np.random.default_rng(nd.SEED)
    torch.manual_seed(nd.SEED)
    ...
    logger.info(
        "pipeline_variant=nodate_train1month_frozen_oos12 prompt_module=roll_xs_bucket_factor_pipeline_nodate "
        "oos_infer=%s",
        args.oos_infer,
    )
    ...
    logger.info("BATCH_SIZE=%d GRAD_ACCUM=%d", nd.BATCH_SIZE, nd.GRAD_ACCUM)

    data = nd.load_data_xs(logger)
    df_all = nd.attach_combo_keys(data.df_events, data.event_cols)
    ...
    try:
        tokenizer = nd.AutoTokenizer.from_pretrained(tok_src, use_fast=True, **tok_kw)
    except Exception:
        tokenizer = nd.AutoTokenizer.from_pretrained(tok_src, use_fast=False, **tok_kw)
    ...
    if bc.use_image_text_to_text:
        ...
        base_lm = nd.AutoModelForCausalLM.from_pretrained(bc.model_id, **lm_kw)
    ...
    lora_cfg = nd.LoraConfig(
        r=8,
        lora_alpha=16,
        lora_dropout=0.05,
        bias="none",
        task_type="CAUSAL_LM",
        target_modules=nd.detect_lora_targets(base_lm),
    )
    if args.resume_ckpt_dir:
        lm = nd.PeftModel.from_pretrained(base_lm, args.resume_ckpt_dir, is_trainable=True)
    else:
        lm = nd.get_peft_model(base_lm, lora_cfg)
    wrapped = nd.LMWithBucketHead(lm, lm.config.hidden_size, nd.N_CLASS)
    ...
    opt = nd.AdamW(
        [
            {"params": [p for p in wrapped.lm.parameters() if p.requires_grad], "lr": nd.LR_LORA},
            {"params": wrapped.head.parameters(), "lr": nd.LR_HEAD},
        ]
    )
    ...
    if args.oos_infer == "combo_cache":
        ...
        va_p0 = start_p + k0 + 1
        te_p0 = start_p + k0 + 2
        df_train = df_all.loc[nd.month_mask(df_all, tr_p0)].copy()
        df_val = df_all.loc[nd.month_mask(df_all, va_p0)].copy()
        df_test0 = df_all.loc[nd.month_mask(df_all, te_p0)].copy()
        if df_train.empty or df_val.empty or df_test0.empty:
            logger.error(
                "Missing data for roll %d (train=%s val=%s test=%s)", k0 + 1, tr_p0, va_p0, te_p0
            )
        else:
            lab_tr = nd.build_combo_day_labels(df_train, ret_str, nd.MIN_UNIV, nd.MIN_COMBO_STOCKS)
            lab_va = nd.build_combo_day_labels(df_val, ret_str, nd.MIN_UNIV, nd.MIN_COMBO_STOCKS)
            if len(lab_tr) < 100:
                logger.error("Too few train labels at roll %d: %d", k0 + 1, len(lab_tr))
            else:
                if len(lab_tr) > nd.MAX_TRAIN_SAMPLES:
                    lab_tr = lab_tr.sample(
                        n=nd.MAX_TRAIN_SAMPLES, random_state=int(rng.integers(0, 2**31 - 1))
                ...
                for k in range(args.n_rolls):
                    ...
                    va_p = start_p + k + 1
                    te_p = start_p + k + 2
                    df_test = df_all.loc[nd.month_mask(df_all, te_p)].copy()
                    if df_test.empty:
                ...
                if test_frames:
                    ...
                    _bs = nd.BATCH_SIZE
                    nd.BATCH_SIZE = args.infer_batch_size
                    try:
                        wrapped.eval()
        ...
        for k in range(args.n_rolls):
            ...
            df_train = df_all.loc[nd.month_mask(df_all, tr_p)].copy()
            df_val = df_all.loc[nd.month_mask(df_all, va_p)].copy()
            df_test = df_all.loc[nd.month_mask(df_all, te_p)].copy()
            ...
            pred_df = nd.predict_factors_for_test(wrapped, df_test, tokenizer, args.backend, pad_id, device)
            pred_df = pred_df[["Date", "Stock", "Factor", "T0_T1_RETURN"]].copy()
            pred_df["roll"] = k + 1
    ...
    test_ic = nd.compute_mean_daily_ic_factor(final_results, factor_col="Factor")
    ...
    test_ic_tb = nd.compute_mean_daily_ic_factor_top_bottom(final_results, factor_col="Factor", tail_pct=0.05)
    ...
    if args.oos_infer == "combo_cache":
        ...
        for _k in range(args.n_rolls):
            te_p = start_p + _k + 2
            dft = df_all.loc[nd.month_mask(df_all, te_p)]
            if not dft.empty:
    ...
    with open(meta_json, "w", encoding="utf-8") as f:
        json.dump(
            {
                "pipeline_variant": "nodate_train1month_frozen_oos12",
